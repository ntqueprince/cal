...
// Fasali Constants from logic file
const MONTHS_FASALI = [
...
function initializeDateRangeDropdowns() {
    ...
    // Populate "From" Months
    MONTHS_FASALI.forEach(m => {
        fromMonth.add(new Option(m.text, m.value));
    });

    // Populate "From" Days
    for (let i = 1; i <= 15; i++) {
        fromDay.add(new Option(i, i));
    }

    // Set Automatic "To" Date values
...
function selectMode(mode) {
    ...
    document.querySelectorAll('.radio-indicator').forEach(ind => {
        ind.innerHTML = '<div class="w-4 h-4 border-2 border-gray-300 rounded-full"></div>';
    });
    input.parentElement.querySelector('.radio-indicator').innerHTML = 
...
function displayResults(result, principal, rate, years, months, mode) {
    ...
    if (result.breakdown.length > 1) {
        html += '<div class="text-xs text-gray-600 mt-3 mb-2 font-bold">YEARLY COMPOUNDING BREAKDOWN:</div>';
        result.breakdown.forEach(item => {
            html += `<div class="breakdown-item text-sm py-2">${item}</div>`;
        });
    }

    html += `
        <div class="breakdown-item bg-purple-50 border-purple-600 mt-4">
            <div class="flex justify-between text-purple-800"><span>Total Interest:</span><b class="text-lg">₹${result.interest.toFixed(2)}</b></div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('setDateRangeBtn').addEventListener('click', () => {
        ...
        if (!fY || !fM || !fP || !fD) {
            alert("Please select all FROM date fields.");
            return;
        }
    ...
    selectMode('custom'); // Default mode
});
